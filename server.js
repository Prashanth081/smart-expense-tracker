...
// Initialize SQLite Database with Absolute Path to avoid working directory issues
const dbPath = path.resolve(__dirname, 'database.sqlite');
const db = new sqlite3.Database(dbPath, (err) => {
    if (err) {
        console.error('CRITICAL: Error opening database at', dbPath, err);
    } else {
        ...
        db.run(`CREATE TABLE IF NOT EXISTS credit_cards (
            ...
            db.run("ALTER TABLE credit_cards ADD COLUMN next_payment_date TEXT", (err) => { });
        });
        ...
        db.run(`CREATE TABLE IF NOT EXISTS loans (
            ...
            db.run("ALTER TABLE loans ADD COLUMN interest_rate REAL", (err) => { });
            db.run("ALTER TABLE loans ADD COLUMN next_emi_date TEXT", (err) => { });
        });
...
function processRecurringLogic(callback) {
    ...
    db.all("SELECT * FROM recurring_subscriptions WHERE next_date <= ?", [todayStr], (err, subs) => {
        ...
        if (!subs || subs.length === 0) return callback();

        let pending = subs.length;
        console.log(`Processing ${pending} recurring subscriptions...`);

        subs.forEach(sub => {
            const txn_id = 'txn-rec-' + Date.now() + '-' + Math.floor(Math.random() * 1000);
            ...
            db.run("INSERT INTO transactions (txn_id, date, merchant, category, amount, type, notes) VALUES (?, ?, ?, ?, ?, ?, ?)",
                [txn_id, sub.next_date, sub.merchant, sub.category, sub.amount, sub.type, "Auto-generated recurring entry"], (err) => {
                    ...
                    if (err) {
                        console.error("Failed to insert recurring transaction:", err);
                        // We continue to next step even if this fails to avoid hanging
                    }

                    // Update Balance
                    db.get("SELECT balance, safeToSpend FROM summary WHERE id = 1", (err, row) => {
                        if (!err && row) {
                            const newBalance = row.balance + sub.amount;
                            const newSafe = sub.type === 'expense' ? Math.max(0, row.safeToSpend + sub.amount) : row.safeToSpend;
                    ...
                    if (!isNaN(nextDateObj.getTime())) {
                        ...
                        db.run("UPDATE recurring_subscriptions SET next_date = ? WHERE id = ?", [newNextStr, sub.id], (err) => {
                            if (err) console.error("Failed to update next_date for subscription:", err);
                            pending--;
                            if (pending === 0) {
                                console.log("Finished processing recurring subscriptions.");
                                callback();
                            }
                        });
                    } else {
                        console.error("Invalid next_date for sub ID:", sub.id);
                        pending--;
...
app.post('/api/credit-cards', (req, res) => {
    const { card_number, cvv, total_limit, used_limit, access_limit, card_name, next_payment_date } = req.body;
    db.run("INSERT INTO credit_cards (card_number, cvv, total_limit, used_limit, access_limit, card_name, next_payment_date) VALUES (?, ?, ?, ?, ?, ?, ?)",
        [card_number, cvv, total_limit || 0, used_limit || 0, access_limit || 0, card_name || 'My Card', next_payment_date || ''], function (err) {
            if (err) return res.status(500).json({ error: err.message });
...
app.put('/api/credit-cards/:id', (req, res) => {
    const { card_number, cvv, total_limit, used_limit, access_limit, card_name, next_payment_date } = req.body;
    db.run("UPDATE credit_cards SET card_number=?, cvv=?, total_limit=?, used_limit=?, access_limit=?, card_name=?, next_payment_date=? WHERE id=?",
        [card_number, cvv, total_limit, used_limit, access_limit, card_name, next_payment_date, req.params.id], (err) => {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ success: true });
        });
});
...
app.post('/api/loans', (req, res) => {
    const { loan_type, total_amount, monthly_emi, months_remaining, loan_name, interest_rate, next_emi_date } = req.body;
    db.run("INSERT INTO loans (loan_type, total_amount, monthly_emi, months_remaining, loan_name, interest_rate, next_emi_date) VALUES (?, ?, ?, ?, ?, ?, ?)",
        [loan_type, total_amount || 0, monthly_emi || 0, months_remaining || 0, loan_name || 'My Loan', interest_rate || 0, next_emi_date || ''], function (err) {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ success: true, id: this.lastID });
        });
});
...
app.put('/api/loans/:id', (req, res) => {
    const { loan_type, total_amount, monthly_emi, months_remaining, loan_name, interest_rate, next_emi_date } = req.body;
    db.run("UPDATE loans SET loan_type=?, total_amount=?, monthly_emi=?, months_remaining=?, loan_name=?, interest_rate=?, next_emi_date=? WHERE id=?",
        [loan_type, total_amount, monthly_emi, months_remaining, loan_name, interest_rate, next_emi_date, req.params.id], (err) => {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ success: true });
        });
});
...
app.post('/api/calendar-events', (req, res) => {
    ...
    db.run("INSERT INTO calendar_events (title, date, amount, type, description) VALUES (?, ?, ?, ?, ?)",
        [title, date, amount || 0, type || 'expense', description || ''], function (err) {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ success: true, id: this.lastID });
        });
});
...
app.put('/api/calendar-events/:id', (req, res) => {
    const { title, date, amount, type, description } = req.body;
    db.run("UPDATE calendar_events SET title=?, date=?, amount=?, type=?, description=? WHERE id=?",
        [title, date, amount, type, description, req.params.id], (err) => {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ success: true });
        });
});
